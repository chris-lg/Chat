...
public class ImageDownLoader {
	...
	public Bitmap downloadImage(final String url) {
		String tUrl = url.replaceAll("[^\\w]", "");
		String saveUrl = tUrl.substring(0, tUrl.length() - 3) + ".jpg";
		final String subUrl = saveUrl;
		final String downUrl = DsConstant.MY_QINIU + url;
		// 在内存缓存中和手机内存中找 若都没有就从网络下载
		...
		if (bitmap != null) {
			...
			getThreadPool().execute(new Runnable() {
				...
				public void run() {
					// 网络获取位图
					Bitmap bitmap = getBitmapFormUrl(downUrl);
					Message msg = handler.obtainMessage();
					LogUtil.LogTest("2头像----->" + bitmap);
					if (bitmap != null) {
						DsApplication.uBitmap = bitmap;
					} else {
					}
					try {
						// 保存在SD卡或者手机目录 下载好图片保存自手机中和app内存下面
	...
	public Bitmap downloadImage(final String url, final MsgBeanTable chat,
		...
		if (bitmap != null) {
			...
			final Handler handler = new Handler() {
				...
				public void handleMessage(Message msg) {
					super.handleMessage(msg);
					listener.onChatImageLoader((Bitmap) msg.obj, url, chat);
				}
			};
			getThreadPool().execute(new Runnable() {
				@Override
				public void run() {
					...
					if (bitmap != null) {
						...
						msg.what = DsConstant.HANDLER_QINIU_NET_DOWNLOAD_FAIL;
					} else {
						chat.setLoadState(DsConstant.PUSH_LOAD_FAIL);
						msg.what = DsConstant.HANDLER_QINIU_NET_DOWNLOAD_PIC_OK;
					}
					msg.obj = bitmap;
	...
	public interface onImageLoaderListener {
		// 聊天时候的图片下载
		void onChatImageLoader(Bitmap bitmap, String url, MsgBeanTable chat);
